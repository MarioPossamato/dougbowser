...
class Ui_Form(object):
    ...
    def HandleOpenFromFile(self):
        ...
        with open(CoursePath,'rb') as Course:
            ...
            CourseName = buffer[0xF4+header_size:0xF4+header_size+0x42].decode('utf-16')
            self.CourseName.setText(CourseName)
            CourseDescription = buffer[0x136+header_size:0x136+header_size+0xCA].decode('utf-16')
            self.CourseDescription.setText(CourseDescription)
            TimeLimit = buffer[0x4+header_size:0x4+header_size+0x4]
            ...
            SaveYear = buffer[0x8+header_size:0x8+header_size+0x2]
            ...
            for i in range(2600):
                entity_id = buffer[0x260+header_size+0x20*i:0x260+header_size+0x3+0x20*i]
    ...
    def HandleSave(self):
        ...
        if not CoursePath:
            ...
            buffer[0x4+header_size:0x6+header_size] = TimeLimit
            buffer[0x6+header_size:0x7+header_size] = Autoscroll
            buffer[0x8+header_size:0xA+header_size] = SaveYear
            buffer[0xA+header_size:0xB+header_size] = SaveMonth
            buffer[0xB+header_size:0xC+header_size] = SaveDay
            buffer[0xC+header_size:0xD+header_size] = SaveHour
            buffer[0xD+header_size:0xE+header_size] = SaveMinute
            with open(CoursePath,'wb') as Course:
                Course.write(buffer)

    def EntityNumberBox_Index_Changed(self):
        global buffer
        ParentFlags = buffer[0x254+header_size+0x20*(int(self.EntityNumberBox.currentText())-1):0x254+header_size+0x20*(int(self.EntityNumberBox.currentText())-1)+4]
        ParentFlags = bytearray(ParentFlags)
        ParentFlags.reverse()
        ParentFlags = bytes(ParentFlags)
        ParentFlags = bytes.hex(ParentFlags)
        ChildFlags = buffer[0x258+header_size+0x20*(int(self.EntityNumberBox.currentText())-1):0x258+header_size+0x20*(int(self.EntityNumberBox.currentText())-1)+4]
        ChildFlags = bytearray(ChildFlags)
        ChildFlags.reverse()
        ChildFlags = bytes(ChildFlags)
        ...
        EntityID = buffer[0x260+header_size+0x20*(int(self.EntityNumberBox.currentText())-1):0x260+header_size+0x20*(int(self.EntityNumberBox.currentText())-1)+1]
